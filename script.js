...
(function initHeroScene() {"
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    const color = new THREE.Color(Math.random() > 0.4 ? 0xffffff : 0xea580c);
    colors[i3] = color.r;
    colors[i3 + 1] = color.g;
    colors[i3 + 2] = color.b;
  ...
  // --- CUSTOM THREE.JS CUSTOM GEOMETRY INJECTION ---
    ...
    // Custom gears made from cylinders with smaller cylinder teeth
    function createGear(color) {
      const gearGroup = new THREE.Group();
      const mainGeo = new THREE.CylinderGeometry(1.3, 1.3, 0.5, 12);
      ...
      const center = new THREE.Mesh(mainGeo, mat);
      gearGroup.add(center);

      // Add teeth
      const teethCount = 8;
      for (let t = 0; t < teethCount; t++) {
        const toothGeo = new THREE.BoxGeometry(0.4, 0.5, 0.4);
    ...
    for (let i = 0; i < 8; i++) {
      ...
      gears.push({
        mesh: gear,
        rotSpeed: { x: (Math.random() - 0.5) * 0.01, y: (Math.random() - 0.5) * 0.01, z: 0.02 },
  ...
  const dirLight1 = new THREE.DirectionalLight(0xea580c, 1.2);
  dirLight1.position.set(12, 18, 10);
  scene.add(dirLight1);

  const dirLight2 = new THREE.DirectionalLight(0x64748b, 0.8);
  dirLight2.position.set(-10, -5, -5);
  scene.add(dirLight2);

  const pointLight = new THREE.PointLight(0xea580c, 1.0, 45);
  ...
  function animate() {
    ...
    gears.forEach(g => {
      ...
      g.mesh.position.y = g.baseY + Math.sin(elapsed * g.floatSpeed) * g.floatAmp;
    });
